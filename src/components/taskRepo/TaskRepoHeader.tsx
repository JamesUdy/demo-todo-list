...
const TaskRepoHeader = () => {
  ...
  return (
    <div className='taskHeader-container'>
        ...
        {/* <span className='taskHeader-info' onClick={() => setIsOpen(true)} onMouseEnter={() => setIsPopoverOpen(true)} onMouseLeave={() => setIsPopoverOpen(false)}>
            <Info/>
            <span className={`infoPopup ${isPopoverOpen ? 'block' : 'hidden'}`}>User&nbsp;Guide</span>
            <span className={`infoArrow ${isPopoverOpen ? 'block' : 'hidden'}`}><LeftArrow/></span>
        </span>
